...
ZTEST(fusain_packets, test_create_set_mode)
{
  ...
  helios_create_set_mode(&packet, HELIOS_MODE_HEAT, 2500);

  zassert_equal(packet.msg_type, HELIOS_MSG_STATE_COMMAND, "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_set_mode_t),
      "Length should match");

  helios_cmd_set_mode_t* cmd = (helios_cmd_set_mode_t*)packet.payload;
  zassert_equal(cmd->mode, HELIOS_MODE_HEAT, "Mode should match");
...
{
  helios_packet_t packet;
  helios_create_set_pump_rate(&packet, 250);

  zassert_equal(packet.msg_type, HELIOS_MSG_PUMP_COMMAND, "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_set_pump_rate_t),
      "Length should match");
...
{
  helios_packet_t packet;
  helios_create_set_target_rpm(&packet, 3000);

  zassert_equal(packet.msg_type, HELIOS_MSG_MOTOR_COMMAND, "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_set_target_rpm_t),
      "Length should match");
...
{
  helios_packet_t packet;
  helios_create_glow_command(&packet, 0, 30000);

  zassert_equal(packet.msg_type, HELIOS_MSG_GLOW_COMMAND, "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_glow_t), "Length should match");
...
ZTEST(fusain_packets, test_create_ping_request)
{
  ...
  zassert_equal(packet.msg_type, HELIOS_MSG_PING_REQUEST, "Type should match");
...
{
  helios_packet_t packet;
  helios_create_telemetry_config(&packet, true, 100, 0);

  zassert_equal(packet.msg_type, HELIOS_MSG_TELEMETRY_CONFIG,
      "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_telemetry_config_t),
      "Length should match");
...
ZTEST(fusain_packets, test_create_motor_config)
{
  ...
  zassert_equal(packet.msg_type, HELIOS_MSG_MOTOR_CONFIG, "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_motor_config_t),
      "Length should match");

  helios_cmd_motor_config_t* cmd = (helios_cmd_motor_config_t*)packet.payload;
...
ZTEST(fusain_packets, test_create_pump_config)
{
  ...
  zassert_equal(packet.msg_type, HELIOS_MSG_PUMP_CONFIG, "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_pump_config_t),
      "Length should match");
}
...
ZTEST(fusain_packets, test_create_temp_config)
{
  ...
  zassert_equal(packet.msg_type, HELIOS_MSG_TEMP_CONFIG, "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_temp_config_t),
      "Length should match");
}
...
ZTEST(fusain_packets, test_create_glow_config)
{
  ...
  zassert_equal(packet.msg_type, HELIOS_MSG_GLOW_CONFIG, "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_glow_config_t),
      "Length should match");
}
...
{
  helios_packet_t packet;
  helios_create_data_subscription(&packet, 0x123456789ABCDEF0ULL, 0xFFULL);

  zassert_equal(packet.msg_type, HELIOS_MSG_DATA_SUBSCRIPTION,
      "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_data_subscription_t),
      "Length should match");

  helios_cmd_data_subscription_t* cmd = (helios_cmd_data_subscription_t*)packet.payload;
...
ZTEST(fusain_packets, test_create_data_unsubscription)
{
  helios_packet_t packet;
  helios_create_data_unsubscription(&packet, 0x123456789ABCDEF0ULL);

  zassert_equal(packet.msg_type, HELIOS_MSG_DATA_UNSUBSCRIPTION,
      "Type should match");
  zassert_equal(packet.length, sizeof(helios_cmd_data_unsubscription_t),
      "Length should match");
}
...
ZTEST(fusain_packets, test_create_discovery_request)
{
  ...
  zassert_equal(packet.msg_type, HELIOS_MSG_DISCOVERY_REQUEST,
      "Type should match");
...
{
  helios_packet_t packet;
  helios_create_state_data(&packet, HELIOS_STATE_HEATING, HELIOS_ERROR_NONE);

  zassert_equal(packet.msg_type, HELIOS_MSG_STATE_DATA, "Type should match");
  zassert_equal(packet.length, sizeof(helios_data_state_t),
      "Length should match");

  helios_data_state_t* data = (helios_data_state_t*)packet.payload;
  zassert_equal(data->state, HELIOS_STATE_HEATING, "State should match");
  zassert_equal(data->error, HELIOS_ERROR_NONE, "Error should match");
}

ZTEST(fusain_packets, test_create_ping_response)
{
  helios_packet_t packet;
  helios_create_ping_response(&packet, 123456789ULL);

  zassert_equal(packet.msg_type, HELIOS_MSG_PING_RESPONSE, "Type should match");
  zassert_equal(packet.length, sizeof(helios_data_ping_response_t),
      "Length should match");

  helios_data_ping_response_t* data = (helios_data_ping_response_t*)packet.payload;
...
{
  helios_packet_t packet;
  helios_create_device_announce(&packet, 0x01, 0xFF);

  zassert_equal(packet.msg_type, HELIOS_MSG_DEVICE_ANNOUNCE,
      "Type should match");
  zassert_equal(packet.length, sizeof(helios_data_device_announce_t),
      "Length should match");

  helios_data_device_announce_t* data = (helios_data_device_announce_t*)packet.payload;
...
{
  helios_telemetry_motor_t motors[2] = {
  ...
  helios_packet_t packet;
  int ret = helios_create_telemetry_bundle(&packet, HELIOS_STATE_HEATING,
      HELIOS_ERROR_NONE, motors, 2, temps, 1);

  zassert_equal(ret, 0, "Bundle creation should succeed");
  zassert_equal(packet.msg_type, HELIOS_MSG_TELEMETRY_BUNDLE,
      "Type should match");
...
ZTEST(fusain_packets, test_packet_roundtrip)
{
  ...
  for (int test_idx = 0; test_idx < 5; test_idx++) {
    ...
    switch (test_idx) {
    case 0:
      helios_create_ping_request(&tx_packet);
      break;
    case 1:
      helios_create_set_mode(&tx_packet, HELIOS_MODE_HEAT, 2500);
      break;
    case 2:
      helios_create_state_data(&tx_packet, HELIOS_STATE_HEATING,
          HELIOS_ERROR_NONE);
      break;
    case 3:
      helios_create_discovery_request(&tx_packet);
      break;
    case 4:
      helios_create_device_announce(&tx_packet, 1, 0xFF);
      break;
    }

    /* Encode and decode */
    uint8_t buffer[HELIOS_MAX_PACKET_SIZE * 2];
    int len = helios_encode_packet(&tx_packet, buffer, sizeof(buffer));
    ...
    zassert_equal(result, HELIOS_DECODE_OK, "%s: Decoding should succeed",
        test_names[test_idx]);
