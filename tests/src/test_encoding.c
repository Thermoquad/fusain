...
ZTEST(fusain_encoding, test_encode_basic)
{
  ...
  zassert_equal(buffer[0], HELIOS_START_BYTE, "First byte should be START");
  zassert_equal(buffer[len - 1], HELIOS_END_BYTE, "Last byte should be END");
}
...
ZTEST(fusain_encoding, test_encode_empty_payload)
{
  helios_packet_t packet = {
    .length = 0,
    .msg_type = HELIOS_MSG_DISCOVERY_REQUEST,
  };
  ...
  zassert_true(len >= HELIOS_MIN_PACKET_SIZE,
...
ZTEST(fusain_encoding, test_encode_max_payload)
{
  ...
  zassert_equal(buffer[0], HELIOS_START_BYTE, "Should start with START byte");
  zassert_equal(buffer[len - 1], HELIOS_END_BYTE, "Should end with END byte");
}
...
ZTEST(fusain_encoding, test_encode_byte_stuffing)
{
  helios_packet_t packet = {
    .length = 3,
    .msg_type = HELIOS_START_BYTE, /* Requires escaping */
  };
  packet.payload[0] = HELIOS_START_BYTE; /* Requires escaping */
  packet.payload[1] = HELIOS_END_BYTE; /* Requires escaping */
  packet.payload[2] = HELIOS_ESC_BYTE; /* Requires escaping */
...
ZTEST(fusain_encoding, test_encode_errors)
{
  ...
  /* Test NULL packet */
  int len = helios_encode_packet(NULL, buffer, sizeof(buffer));
  zassert_true(len < 0, "NULL packet should fail");

  /* Test NULL buffer */
  len = helios_encode_packet(&packet, NULL, 100);
  zassert_true(len < 0, "NULL buffer should fail");

  /* Test buffer too small */
  len = helios_encode_packet(&packet, buffer, 2);
  zassert_true(len < 0, "Too small buffer should fail");

  /* Test invalid payload length */
  packet.length = HELIOS_MAX_PAYLOAD_SIZE + 1;
  len = helios_encode_packet(&packet, buffer, sizeof(buffer));
...
ZTEST(fusain_encoding, test_encode_determinism)
{
  helios_packet_t packet = {
    .length = 8,
    .msg_type = HELIOS_MSG_MOTOR_DATA,
  };
  memcpy(packet.payload, "TESTDATA", 8);

  uint8_t buffer1[HELIOS_MAX_PACKET_SIZE * 2];
  uint8_t buffer2[HELIOS_MAX_PACKET_SIZE * 2];

  int len1 = helios_encode_packet(&packet, buffer1, sizeof(buffer1));
  int len2 = helios_encode_packet(&packet, buffer2, sizeof(buffer2));
